...
Some functions appear in the&nbsp;<primitive key="circular" />&nbsp;function that are available as other primitives or combinations of them.
...
* `8∘○` is `⦅√⍤-⋄1⋄+⋄*∘2⦆`
* `¯8∘○` is `⦅-⍤√⍤-⋄1⋄+⋄*∘2⦆`
* `9∘○` is&nbsp;<primitive key="real_part" />
* `10∘○` is &nbsp;<primitive key="magnitude" />
* `¯10∘○` is&nbsp;<primitive key="conjugate" />
* `11∘○` is&nbsp;<primitive key="imaginary_part" />
* `¯11∘○` is&nbsp;<primitive key="pure_imaginary" />
* `12∘○` is&nbsp;<primitive key="phase" />
* `¯12∘○` is&nbsp;<primitive key="unit_polar" />

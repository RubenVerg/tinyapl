...
function PrimitiveLink({ key, short, children }: PrimitiveLinkProps) {
	...
	return <a href={`/docs/primitive/${key}`} class='link-underline link-underline-opacity-0 link-underline-opacity-75-hover'><code>{primitive.glyph}</code> {children ?? (short ? '' : primitive.name)}</a>;
}

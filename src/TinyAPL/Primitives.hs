...
ravel = Function (Just F.ravel') Nothing [G.ravel] Nothing
...
take = Function Nothing (Just F.take') [G.take] Nothing
drop = Function Nothing (Just F.drop') [G.drop] Nothing
left = Function (Just $ \x -> pure x) (Just $ \x _ -> pure x) [G.left] Nothing
right = Function (Just $ \x -> pure x) (Just $ \_ y -> pure y) [G.right] Nothing
iota = Function (Just F.indexGenerator') Nothing [G.iota] Nothing
indices = Function (Just F.indices) Nothing [G.indices] Nothing
...
element = Function (Just F.enlist') Nothing [G.element] Nothing
...
reduce = Adverb
  ...
  , adverbOnFunction = Just $ \f -> pure $ Function (Just $ F.reduce' $ callDyad f) Nothing (makeAdverbRepr (show f) G.reduce) Nothing }
reduceBack = Adverb
  { adverbRepr = [G.reduceBack]
  , adverbContext = Nothing
  , adverbOnArray = Nothing
  , adverbOnFunction = Just $ \f -> pure $ Function (Just $ F.reduceBack' $ callDyad f) Nothing (makeAdverbRepr (show f) G.reduceBack) Nothing }
onPrefixes = Adverb
